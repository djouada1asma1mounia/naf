...
export class DechargesService {
      ...
      <html lang="fr">
          ...
          <body>
              <div class="sheet">
                  <div class="top">
                      <div>
                          <div class="logo">${logoDataUri ? `<img class="logo-img" src="${logoDataUri}" alt="Logo Naftal" />` : 'NAFTAL'}</div>
                          <div class="org">
  ...
  async exportDechargesToExcel(res: Response): Promise<void> {
    const data = await this.findAll();
    const decharges = (data as any).data || [];

    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'Naftal Backend';
    workbook.created = new Date();

    const worksheet = workbook.addWorksheet('Décharges');
    worksheet.columns = [
      { header: 'Référence', key: 'reference', width: 18 },
      { header: 'Type', key: 'type', width: 10 },
      { header: 'Destinataire', key: 'destinataire', width: 30 },
      { header: 'Réceptionnaire', key: 'receptionnaire', width: 24 },
      { header: 'Date', key: 'date', width: 18 },
      { header: 'Nb Items', key: 'count', width: 10 },
    ];

    decharges.forEach((d: any) => {
      worksheet.addRow({
        reference: d.reference || '-',
        type: d.maintenanceType || '-',
        destinataire: d.destinataire || '-',
        receptionnaire: `${d.receptionnairePrenom || ''} ${d.receptionnaireNom || ''}`.trim() || '-',
        date: d.createdAt ? new Date(d.createdAt).toLocaleString('fr-FR') : '-',
        count: Array.isArray(d.items) ? d.items.length : 0,
      });
    });

    const fileName = `decharges-${new Date().toISOString().replace(/[.:]/g, '-')}.xlsx`;
  ...
  async exportDechargesToPdf(res: Response): Promise<void> {
    const data = await this.findAll();
    const decharges = (data as any).data || [];

    const doc = new PDFDocument({ size: 'A4', margin: 36 });
    const fileName = `decharges-${new Date().toISOString().replace(/[.:]/g, '-')}.pdf`;
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
    doc.pipe(res);

    doc.fontSize(16).text('Liste Décharges', { align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(10).text(`Généré le: ${new Date().toLocaleString('fr-FR')}`);
    doc.moveDown(0.8);

    decharges.forEach((d) => {
      doc.fontSize(10).font('Helvetica-Bold').text(d.reference || '-');
      doc.fontSize(9).font('Helvetica').text(`${d.maintenanceType || '-'} • ${d.destinataire || '-'} • ${d.createdAt ? new Date(d.createdAt).toLocaleString('fr-FR') : '-'}`);
      doc.moveDown(0.4);
    });
  ...
  async create(createDechargeDto: CreateDechargeDto, userId?: string) {
    return this.dechargesRepository.manager.transaction(async (manager) => {
      ...
      try {
        await this.materielsService.updateDateSortieForItems(items, savedDecharge.createdAt);
      } catch (err) {
        // ignore errors updating materials to avoid breaking decharge creation
      }
