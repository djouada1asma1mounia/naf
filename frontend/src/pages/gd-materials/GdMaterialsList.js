...
const GdMaterialsList = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!canReadAny) {
      ...
      setOwners([]);
      enqueueSnackbar(
        "Vous n'avez pas la permission de lire les matériels GD (matériels + raisons).",
        { variant: "warning" },
      );
      setLoading(false);
      return;
    ...
    const loaders = [
      ...
      canReadAny || canCreateAny || canUpdateAny
        ? authAPI.getUsers()
        : Promise.resolve([]),
    ];

    const [catsResult, subsResult, ownersResult] =
    ...
    try {
      if (filters.subsidiaryCode) {
        ...
        materialsResult = await materialsAPI.getBySubsidiary(
          filters.subsidiaryCode,
          apiFilters,
        );
      } else if (
        subsResult.status === "fulfilled" &&
        (subsResult.value || []).length > 0
      ) {
        // Aggregate GD materials by each subsidiary code.
        const subsidiaryMaterials = await Promise.allSettled(
          (subsResult.value || []).map((subsidiary) =>
            materialsAPI.getBySubsidiary(subsidiary.code, apiFilters),
          ),
        );
      ...
      setMaterials([]);
      enqueueSnackbar("Impossible de charger les matériels GD.", {
        variant: "warning",
      });
    }
          ...
          (materialsResult || [])
            ...
            .map((item) => [
              item.subsidiaryCode,
              {
                ...
                name: item.subsidiaryName || item.subsidiaryCode,
              },
            ]),
        ).values(),
      );
    ...
    setLoading(false);
  }, [filters, canReadAny, canCreateAny, canUpdateAny, enqueueSnackbar]);
  ...
  const handleExportPdf = async () => {
    if (!canReadAny) {
      enqueueSnackbar(
        "Vous n'avez pas la permission de lire les matériels GD.",
        { variant: "warning" },
      );
      return;
    }
  ...
  const handleFormSubmit = async (data) => {
    try {
      if (editItem && !canUpdateMaterial(editItem)) {
        throw new Error(
          "Vous ne pouvez modifier que vos propres matériels GD.",
        );
      }
      ...
      if (!editItem && !canCreateAny) {
        throw new Error(
          "Vous n'avez pas la permission de créer des matériels GD.",
        );
      }
      ...
      if (editItem) {
        ...
        enqueueSnackbar("GD Material modifié avec succès", {
          variant: "success",
        });
      } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (!target || !canDeleteMaterial(target)) {
        throw new Error(
          "Vous ne pouvez supprimer que vos propres matériels GD.",
        );
      }
  ...
  return (
    <Box>
      <PageHeader
        title="GD Material"
        subtitle={`${filteredMaterials.length} matériel(s) GD trouvé(s)`}
        breadcrumbs={[
              ...
              <Grid item xs={12} sm={6} md={3}>
                <FormControl fullWidth size="small" sx={{ minWidth: 180 }}>
                  ...
                  <Select
                    ...
                    onChange={handleFilterChange("ownerId")}
                    label="Utilisateur"
                  >
                    <MenuItem value="">Tous</MenuItem>
                    {owners.map((o) => (
            ...
            <TableBody>
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <TableRow key={i}>
                    {Array.from({ length: canMutateAny ? 10 : 9 }).map(
                      (_, j) => (
                        <TableCell key={j}>
                          <Skeleton />
                        </TableCell>
