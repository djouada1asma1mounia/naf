...
const toStatusCode = (value) => {
  ...
  if (["A_FAIRE", "EN_COURS", "TERMINE"].includes(normalized))
    return normalized;
  if (normalized === "PLANIFIEE" || normalized === "PLANIFIÉE")
    return "A_FAIRE";
...
const MaintenanceForm = ({
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && initialData) {
      const firstItem = Array.isArray(initialData.items)
        ? initialData.items[0] || {}
        : {};

      // try to find a matching material from loaded materials by inventory, serial or designation
      const matchedMaterial = materials.find((m) => {
        if (!m) return false;
        if (
          firstItem.numeroInventaire &&
          m.inventoryNumber &&
          String(m.inventoryNumber) === String(firstItem.numeroInventaire)
        )
          return true;
        if (
          firstItem.numeroSerie &&
          m.serialNumber &&
          String(m.serialNumber) === String(firstItem.numeroSerie)
        )
          return true;
        if (
          firstItem.designation &&
          m.name &&
          String(m.name).trim() === String(firstItem.designation).trim()
        )
          return true;
        return false;
      });
      ...
      setForm((prev) => ({
        ...initialForm,
        materialId: matchedMaterial
          ? matchedMaterial.id
          : prev.materialId || "",
        interventionType:
          initialData.interventionType || initialData.type || "HARD",
        ...
        itemQuantity:
          Number(firstItem.quantity) > 0 ? Number(firstItem.quantity) : 1,
        itemDesignation:
          firstItem.designation || initialData.materialName || "",
        itemMarque: firstItem.marque || "",
        itemNumeroSerie:
          firstItem.numeroSerie || initialData.materialCode || "",
    ...
    setErrors({});
  }, [open, user, mode, initialData, materials]);
  ...
  const validate = () => {
    const newErrors = {};
    if (mode === "create" && !form.materialId)
      newErrors.materialId = "Champ requis";
    if (!form.destinataire.trim()) newErrors.destinataire = "Champ requis";
    if (!form.interventionnaireNom.trim())
      newErrors.interventionnaireNom = "Champ requis";
    if (!form.interventionnairePrenom.trim())
      newErrors.interventionnairePrenom = "Champ requis";
    if (!form.interventionnaireFonction.trim())
      newErrors.interventionnaireFonction = "Champ requis";
    if (!form.itemDesignation.trim())
      newErrors.itemDesignation = "Champ requis";
    if (!Number(form.itemQuantity) || Number(form.itemQuantity) < 1)
      newErrors.itemQuantity = "Minimum 1";
    setErrors(newErrors);
              ...
              <Autocomplete
                ...
                filterOptions={(options, state) => {
                  ...
                  return options.filter((option) =>
                    getMaterialSearchText(option).includes(query),
                  );
                }}
                  ...
                  <Box component="li" {...props} sx={{ py: 1.2 }}>
                    <Box sx={{ width: "100%" }}>
                      ...
                      >
                        ...
                        {option.subsidiaryCode ? (
                          <Chip
                            size="small"
                            label={`GD ${option.subsidiaryCode}`}
                            color="info"
                          />
                        ) : null}
                      </Box>
                      <Typography
                        ...
                        display="block"
                      >
                        Série: {option.serialNumber || option.code || "N/A"} |
                        Inventaire: {option.inventoryNumber || "N/A"}
                      </Typography>
                    </Box>
                  </Box>
                )}
                renderInput={(params) => (
                  <TextField
                    ...
                    helperText={
                      errors.materialId ||
                      "Recherchez rapidement: désignation, numéro de série, numéro inventaire"
                    }
                  />
                )}
              />
