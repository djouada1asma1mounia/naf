...
const UserForm = ({
  ...
  const togglePermission = (permissionId) => {
    setForm((prev) => {
      const exists = prev.permissionIds.some(
      ...
      <DialogContent>
        ...
        >
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sx={{ minWidth: 70 }}>
                    ...
                    .map((permission) => (
                      ...
                      >
                        <Box display="flex" alignItems="center" gap={1}>
                          <Checkbox
                            size="small"
                            checked={form.permissionIds.some(
                              (value) =>
                                String(value) === String(permission.id),
                            )}
            ...
            {permissions.length === 0 && (
              <Grid item xs={12}>
                <Alert severity="info">
                  Permissions backend indisponibles: la liste des permissions ne
                  peut pas être chargée pour le moment.
                </Alert>
              </Grid>
            )}
          </Grid>
...
const UserManagement = () => {
  ...
  const loadData = useCallback(async () => {
    if (!canRead) {
      ...
      setPermissions([]);
      enqueueSnackbar(
        "Vous n'avez pas la permission de lire les utilisateurs.",
        { variant: "warning" },
      );
      return;
    }
    ...
    setLoading(true);
    try {
      const [usrsResult, deptsResult, rolesResult, permissionsResult] =
      ...
      if (
        ...
        Array.isArray(deptsResult.value)
      ) {
        ...
        if (canCreate || canUpdate) {
          enqueueSnackbar(
            "Liste des départements indisponible. Les actions de création/modification peuvent être limitées.",
            { variant: "info" },
          );
        }
      }
      ...
      if (
        ...
        if (canCreate || canUpdate) {
          enqueueSnackbar(
            "Liste des rôles indisponible. Les actions de création/modification peuvent être limitées.",
            { variant: "info" },
          );
        }
      }
      ...
      if (
        ...
        Array.isArray(permissionsResult.value) &&
  ...
  const handleFormSubmit = async (data) => {
    try {
      if (!data.roleId || Number(data.roleId) <= 0) {
        enqueueSnackbar("Veuillez sélectionner un rôle backend valide.", {
          variant: "error",
        });
        return;
      }

      const permissionSelections = (data.permissionIds || [])
        .map((id) => {
      ...
      if (editItem) {
        ...
        if (createResult?.permissionsApplyWarning) {
          enqueueSnackbar(
            "Compte créé, mais certaines permissions (ex: create-role) n'ont pas pu être appliquées automatiquement. Modifiez le compte pour les réappliquer.",
            { variant: "warning" },
          );
        }
      }
  ...
  return (
    <Box>
        ...
        <CardContent sx={{ py: 2 }}>
          {permissionsFallback && (
            <Alert severity="info" sx={{ mb: 2 }}>
              Les permissions backend sont indisponibles actuellement. Vérifiez
                      ...
                      <Box display="flex" gap={0.5} justifyContent="center">
                        <Tooltip title="Afficher détails">
                          <Button
                            ...
                            onClick={() =>
                              setDetailsDialog({ open: true, user: u })
                            }
                            sx={{
                              textTransform: "none",
                              minWidth: 0,
      ...
      >
          ...
          {detailsDialog.user && (
            <Box sx={{ pt: 0.5 }}>
              ...
              >
                ...
                >
                  {`${detailsDialog.user.firstName?.[0] || ""}${detailsDialog.user.lastName?.[0] || ""}` ||
                    "U"}
                </Avatar>
                <Box>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  >
                    ...
                    <Typography variant="body2" fontWeight={600}>
                      {customRoles.find(
                        (r) =>
                          Number(r.id) === Number(detailsDialog.user.roleId),
                      )?.name ||
                        detailsDialog.user.role ||
                        "—"}
                    </Typography>
                  </Box>
                </Grid>
        ...
        <DialogActions sx={{ px: 3, py: 2 }}>
          <Button
            onClick={() => setDetailsDialog({ open: false, user: null })}
            variant="outlined"
          >
            Fermer
          </Button>
